...
Kd = 8*gain;
% alpha0 = -pi/25;
% dev = [90-21.8279, 90]*(pi/180);     % initial deviation
...
% tspan = [0, 3];
% y0 = [dev(1, 2); 0; dev(1, 1); -3.2];
y0 = [dev(1, 2); 0; dev(1, 1); -3.4];
% y0 = [0.0140-2*alpha0*atan(-0.1769)/pi; 0; 0.0932-3*pi/2; -0.1769];
global flag t_switch q_1 q_2 tau2 q1_ddot q2_ddot
q_1 = zeros(0, 0);
q_2 = zeros(0, 0);
tau2 = zeros(0, 0);
q1_ddot = zeros(0, 0);
...
% Actual q_1 vs time
figure(25)
plot(q_1(:, 1), q_1(:, 2)*180/pi);
% title('Actual q1 vs time')
% saveas(gcf, 'q1act_vs_t.png');
% savefig(gcf, 'q1act_vs_t.fig');

% q1_ddot and q2_ddot vs time
% figure(35)
% plot(q1_ddot(:, 1), [q1_ddot(:, 2) q2_ddot(:, 2)])
% 
% % tau2 vs time
% figure(40)
% plot(tau2(:, 1), tau2(:, 2))
